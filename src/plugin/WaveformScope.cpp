...
void WaveformScope::setMarker(float position, float confidence) { markerPosition = position; markerConfidence = confidence; repaint(); }
void WaveformScope::paint(juce::Graphics& g)
{
    ...
    if (markerPosition >= 0.0f && markerPosition <= 1.0f) { const auto x = b.getX() + juce::roundToInt(markerPosition * static_cast<float>(b.getWidth() - 1)); g.setColour(juce::Colour(0xffe8c547).withAlpha(0.35f + 0.65f * markerConfidence)); g.drawVerticalLine(x, static_cast<float>(b.getY()), static_cast<float>(b.getBottom())); g.fillEllipse(static_cast<float>(x - 4), static_cast<float>(b.getY() + 3), 8.0f, 8.0f); }
}
